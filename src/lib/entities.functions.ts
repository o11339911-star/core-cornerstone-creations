...
export const createEntity = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ entityId: string }> => {
    const { data: entityId, error } = await context.supabase.rpc("create_entity_with_owner", {
      ...
      _legal_name_ar: data.legalNameAr ?? null,
      _legal_name_en: data.legalNameEn ?? null,
      _unified_national_number: data.unifiedNationalNumber ?? null,
      _cr_number: data.crNumber ?? null,
      _tax_number: data.taxNumber ?? null,
      _contact_email: data.contactEmail ?? null,
      _contact_phone: data.contactPhone ?? null,
      _building_no: data.buildingNo ?? null,
      _street: data.street ?? null,
      _district: data.district ?? null,
      _city: data.city ?? null,
      _postal_code: data.postalCode ?? null,
      _additional_no: data.additionalNo ?? null,
      _responsible_name: data.responsibleName ?? null,
      _responsible_title: data.responsibleTitle ?? null,
      _responsible_email: data.responsibleEmail ?? null,
      _responsible_phone: data.responsiblePhone ?? null,
    });
...
export const updateEntityOfficial = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ ok: true }> => {
    const { error } = await context.supabase.rpc("update_entity_official", {
      _entity_id: data.entityId,
      _legal_form: data.legalForm ?? null,
      _legal_name_ar: data.legalNameAr ?? null,
      _legal_name_en: data.legalNameEn ?? null,
      _unified_national_number: data.unifiedNationalNumber ?? null,
      _cr_number: data.crNumber ?? null,
...
export const requestEntityRelationship = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ id: string }> => {
    const { data: id, error } = await context.supabase.rpc("request_entity_relationship", {
      ...
      _ownership_percent: data.ownershipPercent ?? null,
    });
    if (error) throw new Error(mapError(error.message));
