...
begin
  ...
  if _apply_activities then
    ...
    select _entity_id, _version, c, (c = _primary_code), _uid
    ...
    update public.entity_activities a
       set is_primary = (a.activity_code = _primary_code),
           ...
           updated_at = now()
     where a.entity_id = _entity_id
       and (a.is_primary is distinct from (a.activity_code = _primary_code) or a.activity_version is distinct from _version);
  end if;
